...
if [ "${HTTP_CODE}"="200" ]; then
  ...
  npm list -g phantomjs --depth=0 || sudo npm install -g phantomjs@1.9.18 --phantomjs_cdnurl=${Phantomjs_cnpmjs_CDN}
else
  echo "Using bitbucket source to download PhantomJS"
  npm list -g phantomjs --depth=0 || sudo npm install -g phantomjs@1.9.18 --phantomjs_cdnurl=${Phantomjs_bitbucket_CDN}
fi
